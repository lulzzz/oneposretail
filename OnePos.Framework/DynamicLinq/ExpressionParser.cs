...
#pragma warning restore 0219
        ...
        private Expression ParseIdentifier()
        {
            ...
            if (_keywords.TryGetValue(_token.Text, out value))
            {
                ...
                if (valueString == KeywordIt)
                    return ParseIt();
                if (valueString == KeywordIif)
                    return ParseIif();
                if (valueString == KeywordNew)
                    return ParseNew();
                NextToken();
        ...
        private Expression ParseMemberAccess(Type type, Expression instance)
        {
            ...
            if (_token.Id == TokenId.OpenParen)
            {
                ...
                Expression[] args = ParseArgumentList();
                MethodBase mb;
                switch (FindMethod(type, id, instance == null || (type.IsAbstract && type.IsSealed), args, out mb))
                {
                    case 0:
        ...
        private Expression ParseAggregate(Expression instance, Type elementType, string methodName, int errorPos)
        {
            ...
            if (FindMethod(typeof (IEnumerableSignatures), methodName, false, args, out signature) == 0)
        ...
        private void CheckAndPromoteOperand(Type signatures, string opName, ref Expression expr, int errorPos)
        {
            var args = new[] {expr};
            MethodBase method;
            if (FindMethod(signatures, "F", false, args, out method) == 0)
        ...
        private MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess)
        {
            ...
            return (SelfAndBaseTypes(type)
                .Select(t => t.FindMembers(MemberTypes.Property | MemberTypes.Field,
                                           flags,
        ...
        private bool IsApplicable(MethodData method, Expression[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                try
                {
                    promoted = PromoteExpression(args[i], pi.ParameterType, method.MethodBase.Name == "F");
                }
                catch
                {
                    promoted = null;
        ...
        private Expression PromoteExpression(Expression expr, Type type, bool exact)
        {
            ...
            return exact ? null : Expression.Convert(expr, type);
        }
        ...
        {
            if (t1 == t2) return 0;
            if (s == t1 || GetNonNullableType(s) == GetNonNullableType(t1)) return 1;
            if (s == t2 || GetNonNullableType(s) == GetNonNullableType(t2)) return -1;
